...
import steffensen from "../../../utilities/methods/functions/steffensen";
import Graph from "../../graph";
const SteffensenBody = () => {
    ...
    const run = () => {
        ...
        if (validateDataResult) {
            const { iterations, logs } = steffensen(fx, parseFloat(x0), parseFloat(tol), parseFloat(niter))
            setLogs(logs);
            console.log(logs)
    ...
    return (
        <div className="container">
            <h1 className="text-center">Steffensen</h1>
            <SteffensenBodyDescription />
            <SteffensenBodyExecution run={run} clear={clear} dataForm={dataForm} handleChangeDataForm={handleChangeDataForm} logs={logs}/>
            {isRun ? <SteffensenBodyResult columns={columns} rows={rows} extraInfo={extraInfo} /> : null}
            <Graph/>
        </div>
    )
}

export default SteffensenBody;