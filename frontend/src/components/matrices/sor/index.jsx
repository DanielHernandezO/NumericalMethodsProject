...
import SorBodyDescription from "./sorBodyDescription"
import SorBodyExecution from "./sorBodyExecution";
import SorBodyResult from "./sorBodyResult";
import validateVector from "../../../utilities/validateVector";
import validateMatriz from "../../../utilities/validateMatrix";
import sor from "../../../utilities/methods/functions/sor";
...
const SorBody = () => {
    ...
    const validateData = ({ A, b, x0, niter, tol, w}) => {
        ...
        if (isNaN(w)) {
            logsAux.push({ type: 'Error', text: 'w must be a valid number' });
            flag = false;
        }
        if (parseFloat(w) <= 0) {
            logsAux.push({ type: 'Error', text: "w must be greater than 0" });
            flag = false;
        }
        setLogs(logsAux);
        return flag;
    ...
    const run = () => {
        ...
        const {T,C,re,iteration, logs, x} = sor(matrixTransformered,vectorTransformered,vectorxTransformed, parseFloat(dataForm.niter),parseFloat(dataForm.tol),parseFloat(dataForm.w))
    ...
    return (
        <div className="container">
            <h1 className="text-center">Sor</h1>
            <SorBodyDescription/>
            <SorBodyExecution run={run} clear={clear} handleChangeDataForm={handleChangeDataForm} dataForm={dataForm} logs={logs}/>
            {isRun?<SorBodyResult columns={columns} rows={rows} extraInfo={extraInfo} matrixList={matrixList} vectorResult={vectorResult}/>:null}
        </div>
    )
}
export default SorBody;