...
import GaussseidelBodyDescription from "./gaussseidelBodyDescription"
import GaussseidelBodyExecution from "./gaussseidelBodyExecution";
import GaussseidelBodyResult from "./gaussseidelBodyResult";
import validateVector from "../../../utilities/validateVector";
import validateMatriz from "../../../utilities/validateMatrix";
import gaussseidel from "../../../utilities/methods/functions/gaussseidel";
import transformMatrix from "../../../utilities/transformMatrix";
const GaussseidelBody = () => {
    ...
    const run = () => {
        ...
        const {T,C,re,iteration, logs, x} = gaussseidel(matrixTransformered,vectorTransformered,vectorxTransformed, parseFloat(dataForm.niter),parseFloat(dataForm.tol))
    ...
    return (
        <div className="container">
            <h1 className="text-center">Gauss seidel</h1>
            <GaussseidelBodyDescription/>
            <GaussseidelBodyExecution run={run} clear={clear} handleChangeDataForm={handleChangeDataForm} dataForm={dataForm} logs={logs}/>
            {isRun?<GaussseidelBodyResult columns={columns} rows={rows} extraInfo={extraInfo} matrixList={matrixList} vectorResult={vectorResult}/>:null}
        </div>
    )
}
export default GaussseidelBody;