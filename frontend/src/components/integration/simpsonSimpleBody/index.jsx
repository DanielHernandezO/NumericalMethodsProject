...
import simpsonSimple from "../../../utilities/methods/functions/simpsonSimple"
...
const CompoundTrapezeBody = () => {
    ...
    const validateData = ({ fx, a, b, n }) => {
        ...
        if (!isNaN(n) && parseFloat(n) > 0 && ((parseFloat(n) - Math.trunc(n)) === 0) && n%3 !== 0 ) {
        ...
        //Validate a < b
        if(a >= b ){
            logsAux.push({ type: 'Error', text: "a must be less than b" });
            flag = false;
        }
    ...
    const run = () => {
        ...
        if (validateDataResult) {
            const { iterations, A, logs} = simpsonSimple(parseFloat(a), parseFloat(b),fx, parseFloat(n))
    ...
    return (
        <div className="container">
            <h1 className="text-center">Simpson 3/8</h1>
